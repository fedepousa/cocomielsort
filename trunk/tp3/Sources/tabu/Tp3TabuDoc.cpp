...
#define TABU_LONGITUD (v-v/10);
#endif
#ifndef CANT_ITERACIONES
#define CANT_ITERACIONES v*5;
#endif
...
inline void elegir_asig_en_N(int v, vector<bool> &asignacion, int &res, vector< vector< int > > &clausulas, unsigned int &max_iteracion, unsigned int & i_max, const unsigned int &max_iteracion_maxima) {
  ++iteracion;
  for(unsigned int i= 0; i <v; ++i){
      ...
      if(tabu_lit[i]+tabu_longitud <= iteracion || res > max_iteracion_maxima) {
...
int main(){
	...
	#ifdef TIEMPOS
	ofstream tiempos("Tp3TabuDocTiempos.out");
	timeval inicio;
	timeval fin;
	double diferencia;
	#endif
    ...
    asignacion.assign(asig.v,asig.v + asig.n);
		...
		iteracion = 0;
		tabu_longitud = TABU_LONGITUD;	
		tabu_lit = vector<int>(v,-tabu_longitud);
		tabu_lit_neg = vector<int>(v,-tabu_longitud);
		cant_iteraciones = CANT_ITERACIONES;
		iteracion_ultimo_maximo = 0;
